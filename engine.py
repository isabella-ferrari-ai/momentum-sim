# -*- coding: utf-8 -*-
"""执行引擎——热门板块动量轮动。买入：T日收盘选股 → T+1集合竞价(≈开盘价)成交（严格T+1）。
卖出：快信号(止损/止盈/高点回撤)盘中实时随时成交(process_intraday)；
慢信号(概念退出/排名退出/到期)收盘决策、次日开盘卖(process_day)。

每个交易日 D 的处理顺序（process_day，日线回测/收盘结算共用）：
