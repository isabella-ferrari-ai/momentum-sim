...
STOP_LOSS = -0.15            # 成本硬止损：放宽到 -15%（钝化，避免高波动频繁误杀）
TRAIL_STOP = -0.20           # 高点回撤止盈：自持仓最高价回撤 ≤ -20%（放宽）
TRAIL_ARM_PROFIT = 0.20      # 浮盈达 +20% 后才启用高点回撤止盈（放宽）
TAKE_PROFIT = 0.50           # 固定止盈：浮盈 ≥ +50% 落袋（放宽，让趋势走完）
